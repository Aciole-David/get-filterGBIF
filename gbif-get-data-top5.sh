...
awk -v a="$i" '$2==a' FS=$'\t' temp/especies-alvo.txt | LC_ALL=C sort -t$'\t' -k3 -rg | head -5 >>temp/ordenados.txt
awk -v a="$i" '$2==a' FS=$'\t' temp/especies-alvo.txt | LC_ALL=C sort -t$'\t' -k3 -g | head -5 >>temp/ordenados.txt
done


echo -e "Valores maximo e mínimo da coluna 'decimalLatitude' para cada espécie obtido.\n"

cat temp/header-sete-colunas.txt temp/ordenados.txt>Top5-amplitudes.txt
